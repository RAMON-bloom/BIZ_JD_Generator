...
const ExtractionTab: React.FC = () => {
    ...
    const groupedFields = useMemo(() => {
        const byGroup: Record<number, typeof fields> = {};
    ...
    const handleCopyGroup = useCallback((groupId: number, groupFields: typeof fields) => {
        if (!analysisResult) return;
        const text = groupFields.map(f => {
            const val = analysisResult[f.id];
            if (!val) return '';
                                ...
                                {groupedFields.map(({ group, fields: groupFields }) => (
                                    <div key={group.id} className="relative bg-slate-50/70 dark:bg-slate-900/40 border border-slate-200/70 dark:border-slate-800/70 rounded-2xl p-5 space-y-4">
                                        ...
                                        {groupFields.map(field => {
                                            const value = analysisResult[field.id];
